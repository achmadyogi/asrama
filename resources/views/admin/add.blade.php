			...
			<form action="{{ url('/users'.( isset($model) ? "/" . $model->id : "")) }}" method="POST" class="form-horizontal">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						...
						<label for="roles" class="col-sm-2 control-label" style="text-align: center;">
							<input class="col-sm-12" type="checkbox" name="super_check" id="super_check" class="form-control"
							@if (isset($model) && $model['is_superuser'] == 1)
							checked
							@endif
							> <br>
					...
					<div class="row">
						...
						<div class="col-sm-6">
							<select id="asrama" class="form-control" name="asrama">
								...
								@endforeach
								<option
									@if (isset($model) && isset($model->pengelola))
									selected
									@endif
								>Semua Asrama</option>
								@endif
